...
function getGitDiff() {
  try { return execSync('git diff HEAD', { encoding: 'utf8', cwd: ROOT_DIR }) }
  catch { return '' }
}

function getTestOutput(project) {
  const projectRoot = path.resolve(ROOT_DIR, project === 'backend' ? 'backend' : project)
  try {
...
async function main() {
  ...
  for (const { project, taskId } of evaluationTargets) {
    ...
    const taskPath = path.join(ROOT_DIR, project, 'tasks', `${taskId}.md`)
    const criteriaPath = path.join(ROOT_DIR, project, 'tasks', 'criteria', `${taskId}.eval.json`)
    ...
    if (process.env.ANTHROPIC_API_KEY) {
      ...
      try {
        ...
        const statusPath = path.join(ROOT_DIR, project, 'tasks', 'status.json')
